...
export class RollupBuilderConfig {
  ...
  static getCompilerOptionsJSONFollowExtends(filename: string): {
    [key: string]: any;
  } {
    let compopts = {};
  ...
  getTsConfigData() {
    return RollupBuilderConfig.getCompilerOptionsJSONFollowExtends(
      this.getFilePath('tsconfig.json')
    );
  }
...
*/
    ...
    return <RollupOptions>{
      ...
      output: {
        ...
        name:
          this.applicationInfo.name ||
          safeVariableName(this.applicationInfo.name),
        sourcemap:
          tsCompilerOptions?.compilerOptions?.sourceMap &&
          opts.env != 'production',
        exports: 'named',
